...
def compute_tf_idf(train_set, train_labels, dev_set):
    ...
    for wordList in train_set:
        for word in wordList:
            my_list.append(word)

    Train_dict = Counter(my_list)

    for wordList in dev_set:
        finalD = {}
        Dev_dict = Counter(wordList)
        for word in wordList:
            x = np.log10(len(train_set)/( 1 + Train_dict.get(word, 0)))
            y = Dev_dict.get(word , 0)/len(wordList)
            ans = x * y
            finalD[word] = ans

        keyF = list(finalD.keys())
        listF = list(finalD.values())
        ender = keyF[listF.index(max(listF))]

        finalAns.append(ender)
